...
ok(ll_add_fs_rule($ruleset_fd, $LANDLOCK_ACCESS_FS{READ_FILE}, $dh), 'rule added');
$writable_fh->close();
ok(!defined ll_add_fs_rule(fileno(*STDIN), $LANDLOCK_ACCESS_FS{READ_FILE}, $dh), "attempt to add rule to wrong fd: $!");
